"""Upstox full-feed → OrderFlowMap protocol (shared by FastAPI + CLI bridge)."""
...
from app.config.paths import ensure_repo_and_lib_on_path, repo_root
...
class InstrumentResolver:
    ...
    def _resolve_from_master(self, code: str) -> str | None:
        ...
        root = repo_root()
        gz = root / "src" / "server" / "data" / "instruments" / "upstox_complete.json.gz"
    ...
    def _search_symbol(self, sym: str, exch: str) -> str | None:
        ...
        try:
            ...
            self._last_error = f"symbol search: {exc}"
...
class UpstoxFullFeed:
    """Process-wide Upstox V3 ``full`` streamer; fans out OFMap payloads."""

    _instance: "UpstoxFullFeed | None" = None
    ...
    @classmethod
    def shared(cls, token: str) -> "UpstoxFullFeed":
        with cls._guard:
            if cls._instance is None or cls._instance.token != token:
...
def ofmap_api_key_required() -> str:
    """Empty = accept any key from OrderFlowMap authenticate."""
    return (os.environ.get("AK07_OFMAP_API_KEY") or "").strip()


def ofmap_user() -> str:
    return (os.environ.get("AK07_OFMAP_USER") or "AK07").strip() or "AK07"


def build_feed_and_resolver() -> tuple[UpstoxFullFeed, InstrumentResolver]:
    username = ofmap_user()
...
async def handle_ofmap_client(
    ws: Any,
    *,
    feed: UpstoxFullFeed | None = None,
    resolver: InstrumentResolver | None = None,
    api_key_required: str | None = None,
) -> None:
    """OrderFlowMap Live protocol over FastAPI/Starlette WebSocket."""
    if feed is None or resolver is None:
        feed, resolver = build_feed_and_resolver()
    if api_key_required is None:
        api_key_required = ofmap_api_key_required()

    peer = getattr(ws, "client", None)
    logger.info("OFMap client connected: %s", peer)
    authenticated = False
    ...
    async def pump() -> None:
        ...
        while True:
            payload = await q.get()
            await _send({"type": "market_data", "data": payload})
    ...
    try:
        while True:
            ...
            if action == "authenticate":
                ...
                if api_key_required and key != api_key_required:
                    await _send({"message": "Authentication failed"})
                    continue
                authenticated = True
                await _send({"message": "Authentication successful"})
                continue

            if not authenticated:
                # Allow first subscribe after empty-key auth for UX when key unused
                if action == "subscribe" and not api_key_required:
                    authenticated = True
                else:
                    await _send({"message": "Authentication failed"})
                    continue
        ...
        logger.info("OFMap client disconnected (%s): %s", peer, exc)
    finally:
        if sender:
            sender.cancel()
        if instrument_key and q is not None:
            feed.unregister_queue(instrument_key, q)
        logger.info("OFMap client closed: %s", peer)


def ofmap_static_dir() -> Path:
    return repo_root() / "scripts" / "orderflow" / "OrderFlowMap"
