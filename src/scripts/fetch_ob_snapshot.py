...
Run from repo root:
  python src/scripts/fetch_ob_snapshot.py
  python src/scripts/fetch_ob_snapshot.py --json
  python src/scripts/fetch_ob_snapshot.py --user AK07 --scripts CRUDE NIFTY
"""
...
for p in (ROOT, ROOT / "src" / "lib", ROOT / "src" / "bot"):
    s = str(p)
...
def main() -> None:
    ...
    parser.add_argument(
        "--user",
        default="AK07",
        help="Dashboard user whose src/server/data/users/<user>/upstox_credentials.json to use",
    )
