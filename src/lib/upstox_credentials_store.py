"""
Per-user Upstox credentials: src/server/data/users/<username>/upstox_credentials.json
...
# repo/src/lib/this_file.py -> parents[2] == repository root
...
def user_data_dir(username: str) -> Path:
    return REPO_ROOT / "src" / "server" / "data" / "users" / sanitize_username(username)
...
def legacy_admin_bucket_order_logs(username: str) -> list[Path]:
    ...
    for holder in ("AK07", "admin"):
        base = REPO_ROOT / "src" / "server" / "data" / "users" / holder / "archive" / u
        if not base.is_dir():
            continue
